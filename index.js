...
import {
    ...
    buildCharacterAppearancePromptTemplate,
    getPromptStyleForArtStyle,
    getPromptStyleForModel,
    getPromptStyleInstructions,
} from './prompts.js';
...
const defaultSettings = {
    ...
    summarizer_system_prompt_template: DEFAULT_SUMMARIZER_SYSTEM_PROMPT,
 ...
 */
async function summarizeWithAI(text, charName, userName, additionalCharacters = [], contextMessages = null) {
    const settings = extension_settings[extensionName];
    ...
    // Build a structured user prompt when we have multiple messages so the AI
    // can differentiate background context from the trigger scene.
    let userPrompt;
    if (contextMessages && contextMessages.length > 1) {
        const triggerMsg = contextMessages[contextMessages.length - 1];
        const priorMsgs = contextMessages.slice(0, -1);
        const contextBlock = priorMsgs
            .map(m => `[${m.name}]: ${cleanText(m.text)}`)
            .join('\n\n')
            .substring(0, 3000);
        const triggerBlock = cleanText(triggerMsg.text).substring(0, 2000);
        userPrompt = `PRIOR CONTEXT (use to understand the setting, location, ongoing action, mood, and character states — do NOT generate an image for this part):
${contextBlock}

---

TRIGGER MESSAGE (generate the image for this exact moment):
[${triggerMsg.name}]: ${triggerBlock}`;
    } else {
        userPrompt = `Scene to convert into an image prompt:\n\n${cleanedText.substring(0, 5000)}`;
    }
...
async function buildPromptText(prompt, sender = null, messageId = null) {
    ...
    if (messageId !== null || sender !== null) {
        ...
        if (recentMessages.length > 0) {
            ...
            // Pass structured messages when we have more than one, so the summarizer
            // can distinguish prior context from the trigger message.
            if (recentMessages.length > 1) {
                structuredMessages = recentMessages;
            }
        }
    } else if (prompt) {
        rawContent = prompt;
    ...
    if (settings.auto_summarize && canSummarize && rawContent) {
        try {
            ...
            const summary = await summarizeWithAI(cleanedContent, charName, userName, [], structuredMessages);
